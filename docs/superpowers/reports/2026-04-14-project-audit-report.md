...
- `prisma/schema.prisma` targets SQLite and defines `User`, `Team`, `TeamMember`, `InviteCode`, `Task`, `Score`, `Session`, `Announcement`, and `TeamFile` plus `UserRole`/`TaskStatus` enums; many relations cascade (e.g., Team memberships/tasks/files/announcements) while leader/teacher/creator/inviteCode links use `onDelete: Restrict` and the Task assignee uses `SetNull`.
