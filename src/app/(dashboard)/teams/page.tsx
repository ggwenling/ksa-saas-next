...
export default function TeamsPage() {
  ...
  const fetchTeams = useCallback(async () => {
    ...
    try {
      ...
      const json = (await res.json()) as { data?: TeamCard[]; message?: string };
      if (!res.ok) {
        message.error(json.message ?? "获取团队列表失败");
        return;
      }
      setTeams(json.data ?? []);
    } catch {
      message.error("网络异常，获取团队列表失败");
    } finally {
      setLoading(false);
    }
  }, [message]);
  ...
  const createTeam = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        message.error(json.message ?? "创建团队失败");
        return;
      }
      ...
      <Reveal className="glass-panel glass-panel-strong rounded-[34px] px-6 py-6">
        <div className="flex flex-wrap items-end justify-between gap-5">
          <div>
            ...
            <Typography.Title className="!mb-2 !mt-3 !text-3xl">
              团队管理
            </Typography.Title>
